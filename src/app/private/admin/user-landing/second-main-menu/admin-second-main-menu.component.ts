...
export class SecondMainMenuComponent implements OnInit {
    ...
    menus.map((m1: any) => {
      ...
      m1.menus?.map((m2: any) => {
          ...
          m3.menus?.map((m4: any) => {
            ...
            m4.menus?.map((m5: any) => {
              ...
              m5.menus?.map((m6: any) => {
                ...
                m6.level = 6;
                m6.flag =
                  m6.menus?.filter((m7: any) => m7.flag).length > 0 ||
                  this.privilegedMenus.includes(m6.code) ||
                  this.privilegedMenus.includes(m5.code);
              });
              m5.menus = m5.menus?.filter((m6: any) => m6.flag);
              if (m4.menus?.length === 0) delete m4.menus;
              m5.flag =
                m5.menus?.filter((m6: any) => m6.flag).length > 0 ||
                this.privilegedMenus.includes(m5.code) ||
                this.privilegedMenus.includes(m4.code);
            });
            m4.menus = m4.menus?.filter((m5: any) => m5.flag);
            if (m4.menus?.length === 0) delete m4.menus;
            m4.flag =
              m4.menus?.filter((m5: any) => m5.flag).length > 0 ||
              this.privilegedMenus.includes(m4.code) ||
              this.privilegedMenus.includes(m3.code);
          });
          m3.menus = m3.menus?.filter((m4: any) => m4.flag);
          if (m3.menus?.length === 0) delete m3.menus;
          m3.flag =
            m3.menus?.filter((m4: any) => m4.flag).length > 0 ||
            this.privilegedMenus.includes(m3.code) ||
            this.privilegedMenus.includes(m2.code);
        });
        ...
        m2.flag =
          m2.menus?.length > 0 ||
          this.privilegedMenus.includes(m2.code) ||
          this.privilegedMenus.includes(m1.code);
      });
  ...
  async loadMenu() {
    ...
    this.privilegedMenus.push("RetrievePersonList");
    this.privilegedMenus.push("CreateBuyer");
    this.privilegedMenus.push("UpdateBuyer");
    this.privilegedMenus.push("RetrieveBuyer");
    this.privilegedMenus.push("RetrieveBuyerList");
    this.privilegedMenus.push("CreateSeller");
    this.privilegedMenus.push("UpdateSeller");
    this.privilegedMenus.push("RetrieveSeller");
    this.privilegedMenus.push("RetrieveSellerList");
    ...
    this.privilegedMenus.push("CreateEmployeeUpdate");
  ...
  doesServiceBelongsToCodeService(serviceName: string): boolean {
    ...
    if (
      equalsIgnoreCase(serviceName, "templateappui") &&
      this.backendService.isCodeService("templateappui")
    ) {
      return true;
    }
    if (
      equalsIgnoreCase(serviceName, "tempappuischool") &&
      this.backendService.isCodeService("tempappuischool")
    ) {
      return true;
    }
    if (
      equalsIgnoreCase(serviceName, "testservice2ui") &&
      this.backendService.isCodeService("testservice2ui")
    ) {
      return true;
    }
    if (
      equalsIgnoreCase(serviceName, "testschoolfe") &&
      this.backendService.isCodeService("testschoolfe")
    ) {
      return true;
    }
    if (
      equalsIgnoreCase(serviceName, "mockservice1ui") &&
      this.backendService.isCodeService("mockservice1ui")
    ) {
      return true;
    }
    if (
      equalsIgnoreCase(serviceName, "mockservice2ui") &&
      this.backendService.isCodeService("mockservice2ui")
    ) {
      return true;
    }
  ...
  getUserApplicableMenus(userType: any, menus: any[]): any[] {
    ...
    return menus.filter((menu: any) => {
      ...
      if (
        userType !== UserType.STAFF && // Check if empty OR user not in the list
