...
export class UpdateClassInfoComponent  extends FormFieldsTemplateAppImplComponent implements OnInit
{
  ...
  async ngOnInit(): Promise<void>
  {
    ...
    {
      this.retrievedDataObject = await this.backendService.retrieveClassInfo(this.selectedClassInfoUUID);
      this.additionalProperties = await this.getAdditionalPropertiesOnLoad("updateClassInfo", this.toastNotificationService, this.backendService, this.retrievedDataObject);
      this.setSelectedLookupsDataIntoObj(this.retrievedDataObject);
      let updateClassInfoRequestParamList = await this.getApiRequestParameterListCustom('updateClassInfo', this.backendService, this.toastNotificationService);
      this.updateClassInfoSectionFields = this.getSectionFieldsFromApiRequestParams('UpdateClassInfo', updateClassInfoRequestParamList, this);
      this.modifyData("updateClassInfo", this.retrievedDataObject, this.additionalProperties, this.toastNotificationService, this.backendService);
  ...
  setRetrievedObjectInfo(retrievedObjectInfo: any, pageApiName: any)
  {
   setTimeout(() => {
        ...
        this.doAfterPageDataLoaded("updateClassInfo", this.currentRoute, this, retrievedObjectInfo);
  ...
  async updateClassInfo()
  {
    ...
    this.getUpdatedPayload("updateClassInfo", requestModel, this.updateClassInfoSectionData, this.additionalProperties, this.toastNotificationService, this.backendService, this);
    ...
    const entityUpdateResponse: IResponseMessage = await this.backendService.updateClassInfo(requestModel);
    let toastErrorMessage = "Oops, something went wrong !!";
    if(entityUpdateResponse.success == 1)
    {
      if(!this.doAfterSave("updateClassInfo", requestModel, entityUpdateResponse, this, this.router, this.currentRoute, this.backendService, this.toastNotificationService))
      {
        return;
      }
      ...
      if(entityUpdateResponse.errors && entityUpdateResponse.errors.length > 0) {
          ...
          this.populateFieldLevelErrors('updateClassInfo', entityUpdateResponse.errors)
      } else {
