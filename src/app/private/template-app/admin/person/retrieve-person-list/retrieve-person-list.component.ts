...
import {
    retrievePersonListDataObject,
    retrievePersonListSearchFilter,
} from "src/app/shared/interfaces/dto/template-app/person/retrieve-person-list";
  ...
  implements OnInit
{
  ...
  retrievePersonListSectionFields: any = [];
  retrievePersonListSectionData: any = {};
  retrievePersonListSelectOptionsData: any = {};
  ...
  noOfPages: number = 0;
  searchFilter: retrievePersonListSearchFilter = <
    retrievePersonListSearchFilter
  >{};
  pageResultsMap: Map<number, retrievePersonListDataObject[]> = new Map<
    number,
    retrievePersonListDataObject[]
  >();
  ...
  retrievePersonListTableColumns: Array<any> = [];
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.userActions = this.getUserActionsCustom("RetrievePersonList");
    /**
     * <ngb-pagination> component (pageChange) event is fired onload, so need not trigger search() explicitly here.
     */

    //Load table columns
    let retrievePersonListResponseParamList =
      await this.getApiResponseParameterListCustom(
        "retrievePersonList",
        this.backendService,
        this.toastNotificationService
      );
    this.retrievePersonListTableColumns = this.getListApiTableColumnListCustom(
      "retrievePersonList",
      retrievePersonListResponseParamList,
      this
    );
    //Load page fields
    let retrievePersonListRequestParamList =
      await this.getApiRequestParameterListCustom(
        "retrievePersonList",
        this.backendService,
        this.toastNotificationService
      );
    this.retrievePersonListSectionFields =
      this.getSectionFieldsFromApiRequestParams(
        "retrievePersonList",
        retrievePersonListRequestParamList,
        this
      );
  ...
  async resetSearchCriteria() {
    this.retrievePersonListSectionData = {};
  }
  ...
  populateSearchFilterFromForm() {
    this.pageResultsMap.clear();
    this.searchFilter.name = this.retrievePersonListSectionData["name"];
    this.searchFilter.emailId = this.retrievePersonListSectionData["emailId"];
    this.searchFilter.contactNo =
      this.retrievePersonListSectionData["contactNo"];
  ...
  async handlePersonListPageChange(): Promise<void> {
  ...
  async fetchPersonList() {
    ...
    let searchResponse = <RetrieveListResponseModel>(
      await this.backendService.retrievePersonList(this.searchFilter)
    );
