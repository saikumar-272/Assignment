...
@Component({
selector: "app-retrieve-sales-invoice2",
  templateUrl: "./retrieve-sales-invoice2.component.html",
  styleUrls: ["./retrieve-sales-invoice2.component.scss"],
  imports: [BrowserAnimationsModule, DynamicFieldDisplayComponent, CommonModule, RouterModule, NgbModule],
  standalone: true
})
export class RetrieveSalesInvoice2Component
  extends FormFieldsTemplateAppImplComponent
  implements OnInit
{
  pageComponentReference: any;
  pageHeading: any = "Sales Invoice2";
  userActions: Array<any> = [];
  retrieveSalesInvoice2FieldList: Array<any> = [];
  ...
  sectionsShowHideInfo = {
    enableRetrieveSalesInvoice2Section: true,
  };
  lineItemsSectionsShowHideInfo = {};

  additionalProperties: any = {};
  pageSectionNameList: any[] = ["retrieveSalesInvoice2"];
  constructor(
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.retrievedDataObject = await this.backendService.retrieveSalesInvoice2(
      this.selectedSalesInvoice2UUID
    );
    if (
    ...
    this.additionalProperties = await this.getAdditionalPropertiesOnLoad(
      "retrieveSalesInvoice2",
      this.toastNotificationService,
      this.backendService,
      this.retrievedDataObject
    );
    this.userActions = this.getUserActionsCustom("RetrieveSalesInvoice2");

    //Load page fields
    let retrieveSalesInvoice2ResponseParamList =
      await this.getApiResponseParameterListCustom(
        "retrieveSalesInvoice2",
        this.backendService,
        this.toastNotificationService
      );
    this.retrieveSalesInvoice2FieldList =
      this.getRetrievePageSectionFieldListCustom(
        "retrieveSalesInvoice2",
        "RetrieveSalesInvoice2",
        retrieveSalesInvoice2ResponseParamList,
  ...
  executeUserAction(actionName: any) {
    this.executeUserActionCustom(
      ...
      "RetrieveSalesInvoice2",
      this.router,
      { id: this.selectedSalesInvoice2UUID }
    );
  }
