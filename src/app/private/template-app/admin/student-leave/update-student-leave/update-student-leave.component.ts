...
import {
    IRetrieveStudentLeaveDto
} from 'src/app/shared/interfaces/dto/template-app/student-leave/retrieve-student-leave';
import {
    IUpdateStudentLeaveRequestModel
} from 'src/app/shared/interfaces/dto/template-app/student-leave/update-student-leave';
...
export class UpdateStudentLeaveComponent  extends FormFieldsTemplateAppImplComponent implements OnInit
{
  ...
  async ngOnInit(): Promise<void>
  {
    ...
    if (this.selectedStudentLeaveUUID && this.selectedStudentLeaveUUID.length > 0)
    {
      let retrievedObjectInfo: IRetrieveStudentLeaveDto = await this.backendService.retrieveStudentLeave(this.selectedStudentLeaveUUID);
      this.additionalProperties = await this.getAdditionalPropertiesOnLoad("updateStudentLeave", this.toastNotificationService, this.backendService, retrievedObjectInfo);
      this.setSelectedLookupsDataIntoObj(retrievedObjectInfo);
      let updateStudentLeaveRequestParamList = await this.getApiRequestParameterListCustom('updateStudentLeave', this.backendService, this.toastNotificationService);
      this.updateStudentLeaveSectionFields = this.getSectionFieldsFromApiRequestParams('UpdateStudentLeave', updateStudentLeaveRequestParamList);
      this.modifyData("updateStudentLeave", retrievedObjectInfo, this.additionalProperties, this.toastNotificationService, this.backendService);
  ...
  setRetrievedObjectInfo(retrievedObjectInfo:
  IRetrieveStudentLeaveDto
  , pageApiName: any
  )
  {
   setTimeout(() => {
        ...
        this.doAfterPageDataLoaded("updateStudentLeave", this.currentRoute, this, retrievedObjectInfo);
  ...
  async updateStudentLeave()
  {
    ...
    this.getUpdatedPayload("updateStudentLeave", requestModel, this.updateStudentLeaveSectionData, this.additionalProperties, this.toastNotificationService, this.backendService);
    ...
    const entityUpdateResponse: IResponseMessage = await this.backendService.updateStudentLeave(requestModel);
    let toastErrorMessage = "Oops, something went wrong !!";
    if(entityUpdateResponse.success == 1)
    {
      if(!this.doAfterSave("updateStudentLeave", requestModel, entityUpdateResponse, this, this.router, this.currentRoute, this.backendService, this.toastNotificationService))
      {
        return;
      }
      ...
      if(entityUpdateResponse.errors && entityUpdateResponse.errors.length > 0) {
          ...
          this.populateFieldLevelErrors('updateStudentLeave', entityUpdateResponse.errors)
      } else {
