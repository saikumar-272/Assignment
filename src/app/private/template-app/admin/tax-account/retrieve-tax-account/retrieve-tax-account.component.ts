...
@Component({
selector: "app-retrieve-tax-account",
  imports: [RouterModule, CommonModule, DynamicFieldDisplayComponent, NgbModule, FormsModule, ReactiveFormsModule],

  templateUrl: "./retrieve-tax-account.component.html",
  styleUrls: ["./retrieve-tax-account.component.scss"],
  standalone: true
})
export class RetrieveTaxAccountComponent
  extends FormFieldsTemplateAppImplComponent
  implements OnInit
{
  pageComponentReference: any;
  pageHeading: any = "Tax Account";
  userActions: Array<any> = [];
  retrieveTaxAccountFieldList: Array<any> = [];
  ...
  sectionsShowHideInfo = {
    enableRetrieveTaxAccountSection: true,
  };
  lineItemsSectionsShowHideInfo = {};

  additionalProperties: any = {};
  pageSectionNameList: any[] = ["retrieveTaxAccount"];
  constructor(
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (
      !(this.selectedTaxAccountUUID && this.selectedTaxAccountUUID.length > 0)
    ) {
      return;
    }
    ...
    this.userActions = this.getUserActionsCustom("RetrieveTaxAccount");

    //Load page fields
    let retrieveTaxAccountResponseParamList =
      await this.getApiResponseParameterListCustom(
        "retrieveTaxAccount",
        this.backendService,
        this.toastNotificationService
      );
    this.retrieveTaxAccountFieldList =
      this.getRetrievePageSectionFieldListCustom(
        "retrieveTaxAccount",
        "RetrieveTaxAccount",
        retrieveTaxAccountResponseParamList,
  ...
  executeUserAction(actionName: any) {
    this.executeUserActionCustom(
      ...
      "RetrieveTaxAccount",
      this.router,
      { id: this.selectedTaxAccountUUID }
    );
  }
