...
@Component({
selector: "app-retrieve-user-role",
  templateUrl: "./retrieve-user-role.component.html",
  imports: [RouterModule, CommonModule, DynamicFieldDisplayComponent, NgbModule, FormsModule, ReactiveFormsModule],
  styleUrls: ["./retrieve-user-role.component.scss"],
  standalone: true
})
export class RetrieveUserRoleComponent
  extends FormFieldsTemplateAppImplComponent
  implements OnInit
{
  pageComponentReference: any;
  pageHeading: any = "User Role";
  userActions: Array<any> = [];
  retrieveUserRoleFieldList: Array<any> = [];
  ...
  sectionsShowHideInfo = {
    enableRetrieveUserRoleSection: true,
  };
  lineItemsSectionsShowHideInfo = {};

  additionalProperties: any = {};
  pageSectionNameList: any[] = ["retrieveUserRole"];
  constructor(
    ...
    this.currentRoute.queryParams.subscribe((params) => {
      this.selectedUserRoleUUID = params["id"];
    });
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (!(this.selectedUserRoleUUID && this.selectedUserRoleUUID.length > 0)) {
      return;
    }
    this.retrievedDataObject = await this.backendService.retrieveUserRole(
    ...
    this.userActions = this.getUserActionsCustom("RetrieveUserRole");

    //Load page fields
    let retrieveUserRoleResponseParamList =
      await this.getApiResponseParameterListCustom(
        "retrieveUserRole",
        this.backendService,
        this.toastNotificationService
      );
    this.retrieveUserRoleFieldList = this.getRetrievePageSectionFieldListCustom(
      "retrieveUserRole",
      "RetrieveUserRole",
      retrieveUserRoleResponseParamList,
  ...
  executeUserAction(actionName: any) {
    this.executeUserActionCustom(actionName, "RetrieveUserRole", this.router, {
      id: this.selectedUserRoleUUID,
    });
  }
